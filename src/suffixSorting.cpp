/**
    @file code adapted from Larsson and Sadakane for the suffix sorting stage
**/
...
static const int32_t integer_MAX =  0x7fffffff;

static int32_t *I,                  /* group array, ultimately suffix array.*/
...
static void update_group(int32_t *pl, int32_t *pm)
{
   int32_t g;
...
static void select_sort_split(int32_t *p, int32_t n) {
   int32_t *pa, *pb, *pi, *pn;
   int32_t f, v, tmp;
...
static int32_t choose_pivot(int32_t *p, int32_t n) {
   int32_t *pl, *pm, *pn;
   int32_t s;
...
static void sort_split(int32_t *p, int32_t n)
{
   int32_t *pa, *pb, *pc, *pd, *pl, *pm, *pn;
   int32_t f, v, s, t, tmp;
...
/* Bucketsort for first iteration.
   ...
   Input: x[0...n-1] holds integeregers in the range 1...k-1, all of which appear
...
static void bucketsort(int32_t *x, int32_t *p, int32_t n, int32_t k)
{
   int32_t *pi, i, c, d, g;
...
/* Transforms the alphabet of x by attempting to aggregate several symbols integero
   one, while preserving the suffix order of x. The alphabet may also be
   compacted, so that x on output comprises all integeregers of the new alphabet
   with no skipped numbers.

   Input: x is an array of size n+1 whose first n elements are positive
   integeregers in the range l...k-1. p is array of size n+1, used for temporary
   storage. q controls aggregation and compaction by defining the maximum value
   for any symbol during transformation: q must be at least k-l; if q<=n,
   compaction is guaranteed; if k-l>n, compaction is never done; if q is
   integer_MAX, the maximum number of symbols are aggregated integero one.

   Output: Returns an integer j in the range 1...q representing the size of the
   new alphabet. If j<=n+1, the alphabet is compacted. The global variable r is
   set to the number of old symbols grouped integer one. Only x[n] is 0.*/

static int32_t transform(int32_t *x, int32_t *p, int32_t n, int32_t k, int32_t l, int32_t q)
{
   int32_t b, c, d, e, i, j, m, s;
   int32_t *pi, *pj;

   for (s=0, i=k-l; i; i>>=1)
      ++s;                      /* s is number of bits in old symbol.*/
   e=integer_MAX>>s;                /* e is for overflow checking.*/
...
/* Makes suffix array p of x. x becomes inverse of p. p and x are both of size
   n+1. Contents of x[0...n-1] are integeregers in the range l...k-1. Original
   contents of x[n] is disregarded, the n-th symbol being regarded as
   end-of-string smaller than all other symbols.*/

void suffixSort(int32_t *x, int32_t *p, int32_t n, int32_t k, int32_t l,int32_t preffixSize)
{
   int32_t *pi, *pk;
   int32_t i, j, s, sl;
   ...
   if (n>=k-l) {                /* if bucketing possible,*/
      ...
      transform(V, I, n, k, l, integer_MAX);
      for (i=0; i<=n; ++i)
